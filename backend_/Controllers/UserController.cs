...
namespace SecuLink.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> EditUser([FromBody] EditForm user)
        {
            ...
            await _userService.Edit(user.CurrentUsername, user.Username, user.FirstName, user.LastName, user.Role, user.Email);

            var c = await _cardService.SelectBySerialNumber(user.CurrentSerialNumber);

            if (c is null)
                return StatusCode(404);

            await _cardService.Edit(user.SerialNumber, user.CurrentSerialNumber);
        ...
        [HttpPost]
        [Route("changepass")]
        public async Task<IActionResult> ChangePassword([FromBody] ChangePassForm cf)
        {
            var result = await _authService.Authenticate(cf.Token, _tokenService);
            if (result != 200)
                return StatusCode(result);

            var u = await _userService.SelectById((await _tokenService.SelectByContent(cf.Token)).UserId);

            if (Encryptor.GetHashSha256(cf.OldPassword, "P539Kw95nauPbZEymAwl2dT8AKcRFWjQ") != u.Password_Enc)
                return StatusCode(403);

            await _userService.ChangePassword(u.Id, Encryptor.GetHashSha256(cf.NewPassword, "P539Kw95nauPbZEymAwl2dT8AKcRFWjQ"));
        ...
        {
            var result = await _authService.Authenticate(Token, _tokenService);
            if (result != 200)
                return StatusCode(result);

            var u = await _userService.SelectById((await _tokenService.SelectByContent(Token)).UserId);
            var c = await _cardService.SelectByUserId(u.Id);

            Profile p = new() { FirstName = u.FirstName, LastName = u.LastName, SerialNumber = c.SerialNumber, Role = u.Role};
        ...
        public async Task<IActionResult> GetUsers([FromBody] ListForm lf)
        {
            ...
            try
            {
                ...
                for (int i = CurrentState.UsersLS; i < lf.NumOfElements + CurrentState.UsersLS; i++)
                {
                    ...
                    outList.ElementAtOrDefault(j).SerialNumber = (await _cardService.SelectByUserId(outList.ElementAtOrDefault(j).Id)).SerialNumber;
                    j++;
                }
            }
            catch (Exception ex)
